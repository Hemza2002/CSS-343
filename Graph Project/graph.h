...
public:
  ...
  std::map<std::string, Vertex *>::size_type verticesSize() const;
...
private:
  // This creates a std::map object called graph that maps strings
  // to Vertex pointers. This map represents the vertices in the graph.
  map<string, Vertex *> graph;
  // This variable is used to indicate whether the graph is directed or
  // undirected
  bool directional = true;
  // edgesHelperFunction that takes a const
  // string & parameter called label and returns a std::map object that maps
  // strings to integers
  map<string, int> edgeHelperFunction(const string &label) const;
